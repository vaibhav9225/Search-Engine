...
/*
 ...
 An algorithm for suffix stripping, Program, Volume 14, no. 3, page 130-137,
...
public class StemFilter extends TokenFilter {
	...
	public StemFilter(TokenStream stream) throws TokenizerException {
	...
	public void StemmerLogic() throws TokenizerException {
		if (incoming.getCurrent() != null) {
			...
			init = word.toString();
			if (PatternMatcher.getInstance().matchPattern(init, 35)) {
				b = word.getTermBuffer();
				j = init.length();
				add(b, j);
				stem();
				init = toString();
